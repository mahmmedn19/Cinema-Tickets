/*
 * Created by Mohamed Naser on 7/8/23, 3:33 PM, 2023.
 *
 * Copyright (c) 2023 All rights reserved.
 *
 * Last modified: 7/8/23, 3:33 PM
 *
 */

package com.example.cinematickets.utils


object ListOfImages{
    fun listOfImages():List<String>{
        return listOf(
            "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcSacIchFSO2VC_AglxIliw1Mxo4UYFOMVk-wqy9fwvLDAPFfKVB",
            "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcTfVmUE9T5a10i05V5tDJISK-DjEvB36QynjxWRloQrrWcTM1FJ",
            "https://i.pinimg.com/originals/02/87/61/02876144fce74bd2a5f434c60f031989.jpg",
            "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcSacIchFSO2VC_AglxIliw1Mxo4UYFOMVk-wqy9fwvLDAPFfKVB",
            "https://www.boredpanda.com/blog/wp-content/uploads/2022/05/best-thriller-movies-53-628c95ea5bf80__700.jpg",
            "https://i0.wp.com/dropthespotlight.com/wp-content/uploads/2023/05/imaxtransformers.jpg?fit=675%2C1000&ssl=1",
            "https://cdn.shopify.com/s/files/1/0057/3728/3618/products/108b520c55e3c9760f77a06110d6a73b_240x360_crop_center.progressive.jpg?v=1573652543",
            "https://images.fandango.com/ImageRenderer/0/0/redesign/static/img/default_poster.png/0/images/masterrepository/other/ant_man_ver5.jpg",
            "https://themarketingbirds.com/wp-content/uploads/2021/04/unnamed-1.jpg",
            "https://m.media-amazon.com/images/M/MV5BMGIwMjY4ZTItOTZmNC00OTg0LWJlZDQtMGFmZTZhMGVhMDRkXkEyXkFqcGdeQXVyMTUzOTcyODA5._V1_UX140_CR0,0,140,209_AL_.jpg"
        )
    }
}
